...
public class RetrieveService {
  ...
  public RetrieveResponse retrieveAllData(RetrieveRequest request) {
    ...
    String invNo =
      request.getInvoiceNo() != null ? request.getInvoiceNo().trim() : "";
    String blNo = request.getBlNo() != null ? request.getBlNo().trim() : "";
    String vessel =
      request.getVesselName() != null ? request.getVesselName().trim() : "";
    String voyage =
      request.getVoyageNo() != null ? request.getVoyageNo().trim() : "";
    String customer =
      request.getCustomerName() != null ? request.getCustomerName().trim() : "";
    ...
    if (!vessel.isEmpty() && !voyage.isEmpty()) {
      ...
      conditionsPartial.add(
        "p.vessel_name = :vessel AND p.voyage_no = :voyage"
      );
    ...
    if (!conditionsMain.isEmpty()) {
      ...
      queryPartialExclusion
        .append(wherePartial.replace("p.", "i."))
        .append(")");
    ...
    if (
      ...
      String checkBLSql =
        "SELECT bl_no FROM source_system_records WHERE office_code = :locationId AND reference_no = :invNo ORDER BY reference_no";
      List<String> blList = jdbcTemplate.queryForList(
        checkBLSql,
        params,
        String.class
      );
    ...
    List<Map<String, Object>> mainRecords = jdbcTemplate.queryForList(
      queryMain.toString(),
      params
    );
    List<Map<String, Object>> partialRows = jdbcTemplate.queryForList(
      queryPartial.toString(),
      params
    );
    ...
    for (Map<String, Object> row : partialRows) {
      String partialSql =
        "SELECT p.transaction_no, p.bl_no, p.vessel_code, p.vessel_name, p.voyage_no, p.customer_name, p.type, " +
    ...
    if (!mainRecords.isEmpty() || !outstandings.isEmpty()) {
      Map<String, Object> firstRow = !mainRecords.isEmpty()
        ? mainRecords.get(0)
        : outstandings.get(0);
      ...
      List<Map<String, Object>> checkResult = jdbcTemplate.queryForList(
        queryCheck.toString(),
        params
      );
