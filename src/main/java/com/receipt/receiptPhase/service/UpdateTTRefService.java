...
public class UpdateTTRefService {
  ...
  public Map<String, Object> findByTTNo(String ttNo, String locationId) {
    String sql =
      "SELECT r.transaction_No, i.customer_name, r.reference_No, r.currency_code, r.amount, r.paid_invoice_total " +
      "FROM Receipt r " +
  ...
  public void updateTTNo(
    ...
    String sql =
      "UPDATE RECEIPT SET REFERENCE_NO = ? WHERE REFERENCE_NO = ? AND TRANSACTION_NO = ? " +
    ...
    String safeLogId = UUID.randomUUID()
      .toString()
      .replace("-", "")
      .substring(0, 10);

    String auditSql =
      "INSERT INTO RECEIPT_AUDITLOG (log_id, original_cheque_no, transaction_no, new_cheque_no, reason, payment_mode, action_date, action_created_user, office_code) " +
      "VALUES (?, ?, ?, ?, ?, 'T/T', ?, ?, ?)";

    String safeOldNo = (oldNo != null && oldNo.length() > 32)
      ? oldNo.substring(0, 32)
      : oldNo;
    String safeTransNo = (transNo != null && transNo.length() > 20)
      ? transNo.substring(0, 20)
      : transNo;
    String safeNewNo = (newNo != null && newNo.length() > 32)
      ? newNo.substring(0, 32)
      : newNo;
    String safeRemark = (remark != null && remark.length() > 200)
      ? remark.substring(0, 200)
      : remark;
    String safeUser = (userId != null && userId.length() > 30)
