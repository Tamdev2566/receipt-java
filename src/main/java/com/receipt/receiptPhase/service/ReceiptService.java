...
public class ReceiptService {
  ...
  public Map<String, Object> confirmPayment(Map<String, Object> request) {
    ...
    BigDecimal receiptTotal = decimalValue(
      request,
      "receiptTotal",
      "receipt_total",
      "amount"
    );
    BigDecimal bankCharge = decimalValueOrZero(
      request,
      "bankCharge",
      "bank_charge"
    );
    BigDecimal paidInvoiceTotal = decimalValue(
      request,
      "paidInvoiceTotal",
      "paid_invoice_total",
      "invoiceTotal"
    ...
    if (receiptTotal.add(bankCharge).compareTo(paidInvoiceTotal) > 0) {
      throw badRequest(
        "Over payment detected. Please use /api/receipts/over-payment."
      );
    }
  ...
  public List<Map<String, Object>> getAllReceiptsWithActions(
    ...
    List<ReceiptModal> receipts = receiptRepository.findAllReceiptsByLocation(
      locationId.trim()
    );
    ...
    for (ReceiptModal receipt : receipts) {
      ...
      boolean isPaidActive =
        (receipt.getStatus() == null || !receipt.getStatus()) &&
  ...
  private Map<String, Object> savePayment(
    ...
    for (Map<String, Object> invoice : invoices) {
      ...
      invoice.putIfAbsent("locationId", receipt.getLocationId());
    ...
    response.put(
      "message",
      "Payment confirmed successfully. Transaction No: " + transactionNo
    );
    return response;
  }

  private ReceiptModal buildReceipt(
    Map<String, Object> request,
    ...
    receipt.setLocationId(
      requiredString(request, "locationId", "officeCode", "office_code")
    );
    ...
    receipt.setReceiptDate(
      defaultString(request, now, "receiptDate", "receipt_date")
    );
    receipt.setReferenceNo(
    ...
    receipt.setBankCharge(
      decimalValueOrZero(request, "bankCharge", "bank_charge")
    );
    receipt.setPaidInvoiceTotal(
      decimalValue(
        request,
        "paidInvoiceTotal",
        "paid_invoice_total",
    ...
    receipt.setReceiptTotal(
      decimalValue(request, "receiptTotal", "receipt_total", "amount")
    );
    receipt.setBalanceAmount(
      decimalValueOrZero(request, "balanceAmount", "balance_amount", "balance")
    );
    receipt.setPostedToCoda(
      booleanValue(request, "postedToCoda", "posted_to_coda")
    );
    receipt.setStatus(false);
    receipt.setBank(requiredString(request, "bank"));
    receipt.setCreatedDate(now);
    receipt.setCreatedUser(
      stringValue(request, "createdUser", "created_user", "userId", "user_id")
    );
    receipt.setModifiedDate(now);
    receipt.setModifiedUser(
      stringValue(request, "modifiedUser", "modified_user", "userId", "user_id")
    );
    return receipt;
  }
  ...
  private void insertInvoice(
    ...
    MapSqlParameterSource params = baseInvoiceParams(
      transactionNo,
      now,
      invoice
    );
  ...
  private void insertPartial(
    ...
    BigDecimal oldSettlementAmount = decimalValueOrZero(
      invoice,
      "oldSettlementAmount",
      "OldSettlementAmount"
    );
    BigDecimal newSettlementAmount = decimalValueOrZero(
      invoice,
      "settlementAmount",
      "SettlementAmount",
      "settlement_amt"
    );
    BigDecimal outstandingAmount = oldSettlementAmount
      .subtract(newSettlementAmount)
      .abs();
    BigDecimal exchangeRate = decimalValueOrZero(
      invoice,
      "exchangeRate",
      "exrate",
      "Exrate"
    ...
    if ("SGD".equalsIgnoreCase(currency)) {
      ...
      valueDual =
        BigDecimal.ZERO.compareTo(exchangeRate) == 0
          ? BigDecimal.ZERO
          : outstandingAmount.divide(exchangeRate, 4, RoundingMode.HALF_UP);
    } else {
      valueDoc =
        BigDecimal.ZERO.compareTo(exchangeRate) == 0
          ? BigDecimal.ZERO
          : outstandingAmount.multiply(exchangeRate);
  ...
  private MapSqlParameterSource baseInvoiceParams(
    String transactionNo,
    String now,
    Map<String, Object> invoice
  ) {
    return new MapSqlParameterSource()
      ...
      .addValue(
        "officeCode",
        stringValue(invoice, "locationId", "officeCode", "office_code")
      )
      .addValue("blNo", stringValue(invoice, "blNo", "bl_no", "BLNo"))
      .addValue(
        "vesselCode",
        stringValue(invoice, "vesselCode", "vessel_code", "VesselCode")
      )
      .addValue(
        "vesselName",
        stringValue(invoice, "vesselName", "vessel_name", "VesselName")
      )
      .addValue(
        "voyageNo",
        stringValue(invoice, "voyageNo", "voyage_no", "VoyageNo")
      )
      .addValue(
        "customerName",
        stringValue(invoice, "customerName", "customer_name", "CustomerName")
      )
      .addValue("type", stringValue(invoice, "type", "Type"))
      .addValue(
        "referenceDate",
        stringValue(invoice, "referenceDate", "reference_date", "ReferenceDate")
      )
      .addValue(
        "referenceNo",
        stringValue(invoice, "referenceNo", "reference_no", "ReferenceNo")
      )
      ...
      .addValue(
        "partial",
        bitValue(booleanValue(invoice, "partial", "Partial"))
      )
      .addValue(
        "writeOff",
        bitValue(
      ...
      .addValue(
        "createdDate",
        defaultString(invoice, now, "createdDate", "created_date")
      )
      .addValue(
        "createdUser",
        stringValue(invoice, "createdUser", "created_user", "userId", "user_id")
      );
  }
  ...
  private void validateConfirmBalance(
    ...
    if (receiptTotal.add(bankCharge).compareTo(paidInvoiceTotal) < 0) {
      if (
        balance.compareTo(BigDecimal.ONE) <= 0 &&
        !hasWriteoff(invoiceRows(request))
      ) {
        throw badRequest("Please check write-off for this amount.");
      }
      if (
        balance.compareTo(BigDecimal.ONE) > 0 &&
        !hasPartial(invoiceRows(request))
      ) {
        throw badRequest(
          "Invalid balance to make payment. Please make partial payment."
        );
      }
    }
  }
  ...
  private String nextTransactionNo() {
    ...
    String maxNo = receiptRepository.findMaxTransactionNoForDate(
      datePrefix + "%"
    );
  ...
  private boolean hasPartial(List<Map<String, Object>> invoices) {
    return invoices
      .stream()
      .anyMatch(row -> booleanValue(row, "partial", "Partial"));
  }

  private boolean hasWriteoff(List<Map<String, Object>> invoices) {
    return invoices
      .stream()
      .anyMatch(row -> booleanValue(row, "writeoff", "writeOff", "Write-off"));
  }
  ...
  private String sourceFor(Map<String, Object> invoice) {
    ...
    String referenceNo = stringValue(
      invoice,
      "referenceNo",
      "reference_no",
      "ReferenceNo"
    ...
    if (
      referenceNo != null &&
      (referenceNo.startsWith("CI") || referenceNo.startsWith("I"))
    ) {
      return "DocSys";
    }
    return "Doc4All";
  ...
  private BigDecimal decimalValueOrZero(
    Map<String, Object> values,
    String... keys
  ) {
    Object value = firstPresent(values, keys);
