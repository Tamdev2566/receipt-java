...
public class ChequeBoxController {
  ...
  public ResponseEntity<List<Map<String, Object>>> searchChequeNumbers(
    ...
    String sql =
      "SELECT DISTINCT CHEQUE_NO FROM CHEQUE_READER " +
      "WHERE CHEQUE_NO <> '' " +
      "AND office_code = ? " +
      "AND CHEQUE_NO LIKE ? " +
      "ORDER BY CHEQUE_NO";
  ...
  public ResponseEntity<List<Map<String, Object>>> searchFullChequeNumbers(
    @RequestBody Map<String, String> payload
  ) {
    ...
    String sql =
      "SELECT DISTINCT FULL_CHEQUE_NO FROM CHEQUE_READER " +
      "WHERE CHEQUE_NO = ? " +
      "AND office_code = ? " +
    ...
    List<Map<String, Object>> results = jdbcTemplate.query(
      sql,
      new Object[] { chequeNo, locationId.trim(), param },
      (rs, rowNum) -> {
        Map<String, Object> map = new java.util.HashMap<>();
        map.put("id", rowNum + 1);
