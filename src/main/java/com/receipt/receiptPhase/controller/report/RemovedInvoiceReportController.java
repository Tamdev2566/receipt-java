...
public class RemovedInvoiceReportController {
  ...
  public ResponseEntity<List<RemovedInvoiceReport>> getReportData(
    @RequestParam("fromDate") @DateTimeFormat(
      iso = DateTimeFormat.ISO.DATE
    ) LocalDate fromDate,
    @RequestParam("toDate") @DateTimeFormat(
      iso = DateTimeFormat.ISO.DATE
    ) LocalDate toDate
  ) {
    List<RemovedInvoiceReport> data =
      removedInvoiceReportService.getRemovedInvoiceReport(fromDate, toDate);
  ...
  public ResponseEntity<InputStreamResource> downloadCsv(
    ...
    ByteArrayInputStream in = removedInvoiceReportService.generateCsvReport(
      fromDate,
      toDate
    );

    String currentTimestamp = LocalDate.now().format(
