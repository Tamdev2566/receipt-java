...
public class UpdateChequeController {
  ...
  @GetMapping("/search")
  public ResponseEntity<?> getReceiptByCheque(
    @RequestParam String chequeNo,
    @RequestParam String locationId
  ) {
    Map<String, Object> result = chequeService.findByChequeNo(
      chequeNo,
      locationId
    );
    ...
    if (result == null || result.isEmpty()) {
      return ResponseEntity.status(HttpStatus.NOT_FOUND).body(
        Map.of("message", "No records found")
      );
    }
  ...
  @PostMapping("/update")
  public ResponseEntity<Map<String, String>> updateCheque(
    ...
    try {
      ...
      return ResponseEntity.ok(
        Map.of("message", "Cheque Number Updated Successfully")
      );
    } catch (Exception e) {
      return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(
        Map.of("message", "Update Failed: " + e.getMessage())
      );
    }
  }
}
