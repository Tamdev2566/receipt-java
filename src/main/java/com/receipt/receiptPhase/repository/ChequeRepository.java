...
public class ChequeRepository {
  ...
  public String getUserGroup(String uid) {
    ...
    List<Map<String, Object>> rows = userAuthJdbcTemplate.queryForList(
      sql,
      uid
    );
    if (!rows.isEmpty()) {
  ...
  public String getBankNameByCode(String bankCode) {
    String sql =
      "SELECT bank_name FROM master_banks WHERE bank_code = ? AND is_valid = '1'";
    List<Map<String, Object>> rows = receiptJdbcTemplate.queryForList(
      sql,
      bankCode
    );
    if (!rows.isEmpty()) {
  ...
  public boolean checkBankExistsByName(String bankName) {
    String sql =
      "SELECT COUNT(*) FROM master_banks WHERE bank_name = ? AND is_valid = '1'";
    Integer count = receiptJdbcTemplate.queryForObject(
      sql,
      Integer.class,
      bankName
    );
  ...
  public boolean checkDuplicateCheque(String chequeNo, String fullChequeNo) {
    String sql =
      "SELECT COUNT(*) FROM cheque_reader WHERE EXTRACT(YEAR FROM AGE(CURRENT_DATE, date_created::timestamp)) <= 5 " +
  ...
  public void saveCheque(ChequeReaderModel cheque, String locationId) {
    String sql =
      "INSERT INTO cheque_reader (cheque_reader_id, office_code, bound, cheque_no, bank_name, full_cheque_no, scan_user_id, date_created, auto_read, is_valid) " +
      "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

    String chequeReaderId =
      "CR" +
  ...
  public int updateCheque(ChequeReaderModel cheque) {
    ...
    if (
      cheque.getBankName() != null && !cheque.getBankName().trim().isEmpty()
    ) {
      sql.append("bank_name = ?, ");
      params.add(cheque.getBankName().trim());
    }
    if (
      cheque.getChequeNo() != null && !cheque.getChequeNo().trim().isEmpty()
    ) {
      sql.append("cheque_no = ?, ");
      params.add(cheque.getChequeNo().trim());
    }
    if (
      cheque.getFullChequeNo() != null &&
      !cheque.getFullChequeNo().trim().isEmpty()
    ) {
  ...
  public List<ChequeReaderModel> getAllCheques(String locationId) {
    String sql =
      "SELECT cheque_reader_id, cheque_no, bank_name, full_cheque_no, bound, scan_user_id, auto_read, date_created, is_valid " +
  ...
  public List<ChequeReaderModel> getChequesByUserId(
    String uid,
    String locationId
  ) {
    String sql =
    ...
    return receiptJdbcTemplate.query(
      sql,
      new Object[] { uid, locationId },
      new ChequeRowMapper()
    );
  }

  private class ChequeRowMapper
    implements org.springframework.jdbc.core.RowMapper<ChequeReaderModel>
  {

    @Override
    public ChequeReaderModel mapRow(java.sql.ResultSet rs, int rowNum)
