...
public class MasterBankRepository {
  ...
  public int save(MasterBankModel bank) {
    String sql =
      "INSERT INTO master_banks (bank_id, office_code, bank_code, bank_name, bank_description, " +
  ...
  public List<MasterBankModel> getAllBanksData(String locationId) {
    String sql =
      "SELECT * FROM master_banks WHERE office_code = ? AND COALESCE(is_deleted, '0') != '1' ORDER BY date_created DESC";
  ...
  public MasterBankModel findById(String bankId, String locationId) {
    String sql =
      "SELECT * FROM master_banks WHERE bank_id = ? AND office_code = ?";
    List<MasterBankModel> list = jdbcTemplate.query(
      sql,
      new Object[] { bankId, locationId },
      new MasterBankRowMapper()
    );
  ...
  public int update(MasterBankModel bank) {
    String sql =
      "UPDATE master_banks SET office_code = ?, bank_code = ?, bank_name = ?, bank_description = ?, " +
    ...
    return jdbcTemplate.update(
      ...
      bank.getBankDescription(),
      bank.getIsValid(),
      bank.getDateModified(),
      bank.getUserModified(),
      bank.getBankId(),
      bank.getOfficeCode()
    );
  }
  ...
  // RowMapper
  private static class MasterBankRowMapper
    implements RowMapper<MasterBankModel>
  {

    @Override
    public MasterBankModel mapRow(ResultSet rs, int rowNum)
      throws SQLException {
