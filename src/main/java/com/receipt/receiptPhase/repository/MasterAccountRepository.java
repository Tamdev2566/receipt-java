...
public class MasterAccountRepository {
  ...
  public int save(MasterAccountModel account) {
    String sql =
      "INSERT INTO master_accounts (account_id, office_code, account_currency, account_payment_mode, " +
    ...
    return jdbcTemplate.update(
      ...
      account.getAccountPaymentMode(),
      account.getAccountCode(),
      account.getAccountDisplayName(),
      account.getAccountSeq(),
      account.getIsValid(),
      account.getDateCreated(),
      account.getUserCreated()
    );
  }

  public List<MasterAccountModel> findAll(String locationId) {
    String sql =
      "SELECT * FROM master_accounts WHERE office_code = ? AND COALESCE(is_deleted, '0') != '1'";
  ...
  public MasterAccountModel findById(String accountId, String locationId) {
    String sql =
      "SELECT * FROM master_accounts WHERE account_id = ? AND office_code = ? AND COALESCE(is_deleted, '0') != '1'";
    List<MasterAccountModel> list = jdbcTemplate.query(
      sql,
      new Object[] { accountId, locationId },
      new MasterAccountRowMapper()
    );
  ...
  public int update(MasterAccountModel account) {
    String sql =
      "UPDATE master_accounts SET office_code = ?, account_currency = ?, account_payment_mode = ?, " +
  ...
  public int delete(
    ...
    String sql =
      "UPDATE master_accounts SET is_deleted = '1', user_deleted = ?, date_deleted = ? WHERE account_id = ? AND office_code = ?";
    return jdbcTemplate.update(
      sql,
      userDeleted,
      dateDeleted,
  ...
  private static class MasterAccountRowMapper
    implements RowMapper<MasterAccountModel>
  {

    @Override
    public MasterAccountModel mapRow(ResultSet rs, int rowNum)
      throws SQLException {
      ...
      account.setAccountSeq(
        rs.getObject("account_seq") != null ? rs.getInt("account_seq") : null
      );
