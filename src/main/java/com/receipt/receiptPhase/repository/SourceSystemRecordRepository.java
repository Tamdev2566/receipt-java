...
@Repository
public interface SourceSystemRecordRepository
  extends JpaRepository<SourceSystemRecord, String>
{
  @Query(
    value = "SELECT MIN(bl_no) as id, vessel_code as vesselCode, vessel_name as vesselName " +
      "FROM source_system_records " +
      "WHERE office_code = :locationId " +
      "AND (:customerName IS NULL OR :customerName = '' OR customer_name = :customerName) " +
      "AND (:search IS NULL OR :search = '' OR vessel_name LIKE %:search% OR vessel_code LIKE %:search%) " +
      "GROUP BY vessel_code, vessel_name",
    nativeQuery = true
  )
  List<VesselProjection> findVessels(
  ...
  @Query(
    value = "SELECT MIN(bl_no) as id, voyage_no as voyageNo " +
      "FROM source_system_records " +
      "WHERE office_code = :locationId " +
      "AND (:customerName IS NULL OR :customerName = '' OR customer_name = :customerName) " +
  ...
  List<VoyageProjection> findVoyages(
    @Param("customerName") String customerName,
