...
public interface UndoRepository extends JpaRepository<ReceiptModal, String> {
  @Query(
    value = "SELECT DISTINCT r.Transaction_No, r.Transaction_Date, " +
      ...
      "WHERE r.office_code = :locationId AND (:invNo IS NULL OR i.Transaction_No = :invNo) " +
      ...
      "AND (r.Status IS NULL OR r.Status = '0') ORDER BY r.Transaction_No",
    nativeQuery = true
  )
  List<Map<String, Object>> retrieveReceipts(
  ...
  @Query(
    value = "SELECT DISTINCT i2.Transaction_No,i2.Transaction_Date, i2.BL_No, i2.Vessel_Name, i2.Voyage_No, i2.Customer_Name, " +
      ...
      "WHERE r.office_code = :locationId AND (:invNo IS NULL OR i1.Transaction_No = :invNo) " +
      "AND (:chequeNo IS NULL OR r.Reference_No = :chequeNo) " +
      "AND (:blNo IS NULL OR i1.BL_No = :blNo) " +
      "AND (r.Posted_to_coda IS NULL OR r.Posted_to_coda = '0') " +
      "AND (r.Status IS NULL OR r.Status = '0') ORDER BY i2.Transaction_No",
    nativeQuery = true
  )
  List<Map<String, Object>> retrieveInvoices(
    @Param("invNo") String invNo,
    @Param("chequeNo") String chequeNo,
    @Param("blNo") String blNo,
    @Param("locationId") String locationId
  ...
  @Query(
    value = "SELECT p.* FROM partial p INNER JOIN receipt r ON p.transaction_no = r.transaction_no " +
      "WHERE r.office_code = :locationId AND (p.parital_status = '0') " +
      "AND p.transaction_no = :refNo ORDER BY p.transaction_date",
    nativeQuery = true
  )
  List<Map<String, Object>> getPartialDetails(
  ...
  @Modifying
  @Query(
    value = "UPDATE Receipt SET Status = '1', modified_date = :modDate WHERE Transaction_No = :transNo AND office_code = :locationId",
    nativeQuery = true
  )
  void softDeleteReceipt(
